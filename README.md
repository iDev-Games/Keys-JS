...
Keys.js provides a comprehensive API for game development and advanced interactions:

## Core Methods

```javascript
// Check if key is currently pressed
keys.isKeyDown('space')          // true/false
keys.isKeyActive('w')            // alias for isKeyDown

// Get all currently pressed keys
keys.getActiveKeys()             // ['w', 'shift', 'space']

// Get press count (excludes key repeats)
keys.getKeyPressCount('space')   // 5

// Get hold duration in milliseconds
keys.getKeyHoldDuration('shift') // 1234
```
...
keys.getAxis('a', 'd')           // -1, 0, or 1

// Get 2D movement vector
keys.getVector2D('w', 's', 'a', 'd')  // {x: 0, y: 1}
keys.getWASDVector()             // {x: -1, y: 0}
keys.getArrowVector()            // {x: 1, y: -1}

// Check combos
keys.isComboActive('ctrl', 'shift', 'a')  // true/false
```
...
keys.preventDefaults('space', 'up', 'down')

// Allow default behavior
keys.allowDefaults('space')

// Clear all active keys (for pause menus)
keys.clearAllKeys()

// Reset press counters
keys.resetKeyPressCount('space')
keys.resetKeyPressCount()  // reset all
```
...
See [https://codepen.io/collection/gPEpkK](codepen examples) for comprehensive integration examples.
...
<head>
    ...
    <style>
        ...
        .player {
            ...
            /* WASD movement */
            left: calc(50% + (var(--key-d-count, 0) - var(--key-a-count, 0)) * 5px);
            top: calc(50% + (var(--key-s-count, 0) - var(--key-w-count, 0)) * 5px);
...
<body data-keys-prevent-defaults="*">
    <div class="game-world">
        <div class="player"
             data-keys
             data-keys-watch="w,a,s,d,space,shift"
             data-keys-var="true">
        </div>
    </div>
    ...
    <script>
        ...
        function update() {
            // Get input
            const movement = keys.getWASDVector();

            // Check combos
            if (keys.isComboActive('shift', 'space')) {
                console.log('Super jump!');
            }

            // Check hold duration
            const chargeTime = keys.getKeyHoldDuration('shift');
            if (chargeTime > 1000) {
                console.log('Attack charged!');
            }
